...
size_t ComputeDistances(Input input, size_t empty_multiplier)
{
    ...
    for (int i = 0; i < galaxies.size() - 1; i++) {
        ...
        for (int j = i + 1; j < galaxies.size(); j++) {
            auto b = galaxies[j];
            auto distance
                = DistanceBetweenGalaxies(input, a, b, empty_rows, empty_cols, empty_multiplier);
...
size_t PartOne(Input input) { return ComputeDistances(input, 2); }

size_t PartTwo(Input input) { return ComputeDistances(input, 1000000); }